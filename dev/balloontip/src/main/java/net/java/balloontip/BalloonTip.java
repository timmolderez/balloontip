...
public class BalloonTip extends JPanel {
	...
	protected class VisibilityControl {
		...
		public void update() {
			...
			while (i.hasNext()) {
				if (Boolean.FALSE.equals(i.next())) {
	...
	public void setCloseButton (JButton button, boolean permanentClose, boolean noDefault) {
		...
		if (closeButton!=null) {
			...
			for (int i=0,len=actionListeners.length; i<len; i++) {
				closeButton.removeActionListener(actionListeners[i]);
			}
			remove(closeButton);
			closeButton = null;
		}
	...
	private void setupHelper() {
		...
		tabbedPaneListener = new ComponentAdapter() {
			public void componentShown(ComponentEvent e) {
				visibilityControl.setCriteriumAndUpdate("tabShowing",Boolean.TRUE);
